...
let trendingCache: { data: Song[]; ts: number } | null = null;
...
export const getTrendingSongs = async (): Promise<Song[]> => {
  if (trendingCache && Date.now() - trendingCache.ts < CACHE_TTL) {
    return trendingCache.data;
  }
  try {
    const data = await invokeProxy({ path: "search", query: "trending hits 2025", limit: "20" });
    const songs = (data.data?.results || []).map(mapSong);
    trendingCache = { data: songs, ts: Date.now() };
    return songs;
  } catch {
    return trendingCache?.data || [];
  }
...
export const addToHistory = async (userId: string, song: Song) => {
  await supabase.from("music_history" as any).insert({
    user_id: userId, song_id: song.id, song_name: song.name,
    artist_name: song.artist, album_name: song.album, image_url: song.image,
    duration: song.duration, source_url: song.url,
  } as any);
};

export const getHistory = async (userId: string): Promise<Song[]> => {
  const { data } = await supabase.from("music_history" as any).select("*")
    .eq("user_id", userId).order("played_at", { ascending: false }).limit(50);
  return (data || []).map((h: any) => ({
    id: h.song_id, name: h.song_name, artist: h.artist_name,
    album: h.album_name || "", image: h.image_url || "",
    duration: h.duration || 0, url: h.source_url || "",
  }));
};
...
export const getFavorites = async (userId: string): Promise<Song[]> => {
  const { data } = await supabase.from("music_favorites" as any).select("*")
    .eq("user_id", userId).order("created_at", { ascending: false });
  return (data || []).map((f: any) => ({
    id: f.song_id, name: f.song_name, artist: f.artist_name,
    album: f.album_name || "", image: f.image_url || "",
    duration: f.duration || 0, url: f.source_url || "",
  }));
};

export const isFavorite = async (userId: string, songId: string): Promise<boolean> => {
  const { data } = await supabase.from("music_favorites" as any).select("id")
    .eq("user_id", userId).eq("song_id", songId).maybeSingle();
  return !!data;
};
